...
class Basic_Locomotion_DLS_Isaaclab_Node(Node):
    ...
    def compute_rl_control(self):
        ...
        if np.all(locomotion_policy.leg_any_scaled == np.array([0.0, 0.0, 1.0, 1.0])): #rear legs failure
            trajectory_generator_msg.kp[6:] = array('d', [0.0] * 6)
            trajectory_generator_msg.kd[6:] = array('d', [0.0] * 6)
        elif np.all(locomotion_policy.leg_any_scaled == np.array([1.0, 0.0, 0.0, 0.0])): #FL failure for thigh and calg
            trajectory_generator_msg.kp[1:3] = array('d', [0.0] * 2)
            trajectory_generator_msg.kd[1:3] = array('d', [0.0] * 2)
        elif np.all(locomotion_policy.leg_any_scaled == np.array([0.0, 1.0, 0.0, 0.0])): #FR failure for thigh and calg
            trajectory_generator_msg.kp[4:6] = array('d', [0.0] * 2)
            trajectory_generator_msg.kd[4:6] = array('d', [0.0] * 2)
        elif np.all(locomotion_policy.leg_any_scaled == np.array([0.0, 0.0, 1.0, 0.0])): #RL failure for thigh and calg
            trajectory_generator_msg.kp[7:9] = array('d', [0.0] * 2)
            trajectory_generator_msg.kd[7:9] = array('d', [0.0] * 2)
        elif np.all(locomotion_policy.leg_any_scaled == np.array([0.0, 0.0, 0.0, 1.0])): #RR failure for thigh and calg
            trajectory_generator_msg.kp[10:12] = array('d', [0.0] * 2)
            trajectory_generator_msg.kd[10:12] = array('d', [0.0] * 2)
